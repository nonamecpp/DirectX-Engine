...
void print_surface() {
	...
	for (SurfSorter.iterbegin(); !SurfSorter.iterend(); SurfSorter.iternext()) {
		...
		if (CurSurf.state == msgSURFACE_SHOW || CurSurf.state == msgSURFACE_TRANSPARENT)
			DrawSurface2(backbuffer, CurSurf.rect, SURFACE[CurSurf.surface]);
		if (CurSurf.state == msgSURFACE_SHOW || CurSurf.state == msgSURFACE_HIDE_CHOOSEABLE)
			AlgoScreenCoverPicture(CurSurf.rect.left, CurSurf.rect.top, 
				CurSurf.rect.right - CurSurf.rect.left, 
				CurSurf.rect.bottom - CurSurf.rect.top,
				CurSurf.code);

		if (CurSurf.state == msgSPRITE_SHOW || CurSurf.state == msgSPRITE_TRANSPARENT)
			Sprite_Draw_Frame(SPRITE[CurSurf.sprite], CurSurf.rect.left, CurSurf.rect.top,
				CurSurf.pos, SPRITE_INFO[CurSurf.sprite].framew, 
				SPRITE_INFO[CurSurf.sprite].frameh, 
				SPRITE_INFO[CurSurf.sprite].column, 
					CurSurf.trans);
		if (CurSurf.state == msgSPRITE_SHOW || CurSurf.state == msgSPRITE_HIDE_CHOOSEABLE)
			AlgoScreenCoverPicture(CurSurf.rect.left, CurSurf.rect.top, 
				SPRITE_INFO[CurSurf.sprite].framew, SPRITE_INFO[CurSurf.sprite].frameh, 
				CurSurf.code);
		if (CurSurf.state == msgSPRITE_SHOW || CurSurf.state == msgSPRITE_TRANSPARENT)
			FontPrint()
		if (CurSurf.state == msgSPRITE_SHOW || CurSurf.state == msgSPRITE_HIDE_CHOOSEABLE)
